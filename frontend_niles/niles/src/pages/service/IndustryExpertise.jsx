...
const IndustryExpertise = () => {
    ...
    const industryData = async () => {
        try{
            ...
            const industryDatas = response.data.filter(
                (item)=> item.status ==="Publish" && item.page_type === "Industry Expertise"
                
            );
            setIndustry(industryDatas[0]);
        }catch (error){
    ...
    return (
        <>
            <section className='py-20 relative'>
                <img className='absolute h-full w-full top-0 left-0 object-cover' src={industry.slider_image} alt="background" />
            ...
            <section className="py-12">
                <div className="container flex lg:flex-row flex-col justify-between gap-4">
                    <div className='flex flex-col items-start lg:w-5/12 w-full lg:mb-0 mb-5'>
                        <h2 className='lg:text-4xl sm:text-3xl text-2xl font-bold pb-2 mb-6 relative pseudo-border' data-aos="fade-up" data-aos-duration="1000" data-aos-once='true'>{industry.title}</h2>
                        <div className='text-gray-700'>
                            <p className='mb-2' data-aos="fade-right" data-aos-duration="1000" data-aos-once='true' data-aos-delay='200' dangerouslySetInnerHTML={{__html : industry.short_desc }}></p>
                    ...
                    <div className='lg:w-7/12 w-full' data-aos="zoom-in-up" data-aos-duration="1000" data-aos-once='true' data-aos-delay='200'>
                        <img className='w-full' src={industry.bannerimage} alt="industry expertise" />
                    </div>
                </div>
            </section>
        </>
